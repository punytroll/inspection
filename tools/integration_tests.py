...
class Execute(object):
    ...
    def __str__(self):
        return f"Execute({self.program}, {self.arguments})"
    ...
    while True:
        ...
        try:
            ...
            if type(test) == str:
                ...
                try:
                    result = subprocess.run([test.setup.execute.program] + test.setup.execute.arguments, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                        ...
                        for node in execute_element.childNodes:
                            if node.nodeType == Node.ELEMENT_NODE:
                                if node.tagName == "program":
                                    test.setup.execute.program = get_text(node.childNodes)
    ...
    while True:
        ...
        if finished_test == None:
            ...
            if finished_test.setup.description != None:
                ...
                print()
            print(f"    Running \"{finished_test.setup.execute.program} {get_space_appended(finished_test.setup.execute.arguments)}\"")
    ...
    if test_suite_statistics.number_of_failed_tests > 0:
        ...
        for finished_test in finished_tests_list:
            if finished_test.this_run.statistics.success == False:
                ...
                reasons = []
                if finished_test.this_run.return_code == None:
                    reasons.append("could not be run")
                else:
